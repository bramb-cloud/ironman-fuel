...
export default function FarmTab() {
  ...
  async function collectProduction() {
    ...
    const updated = {
      ...
      // Reset each animal's individual timer
      animals: farm.animals.map(a => ({ ...a, lastCollected: now })),
      last_production_check: now,
    }
  ...
  async function confirmBuyAnimal(type: string, name: string) {
    ...
    const newAnimal = {
      ...
      lastCollected: new Date().toISOString(), // timer starts from purchase
      x: 20 + Math.random() * 60,
      y: 20 + Math.random() * 60,
